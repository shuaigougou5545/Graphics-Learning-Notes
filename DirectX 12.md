...
- 创建`ID3D12Device`，设备代表一个显示适配器(也就是显卡)
- 创建`Fence`并查询描述符大小
- 检测对4X MSAA质量级别的支持
- 创建命令队列和命令列表（命令分配器）
- 创建交换链：缓冲区的一些设置、窗口等
- 创建描述符堆
- 创建渲染目标视图（RTV）：`GetBuffer()`、`CreateRenderTargetView()`
- 创建深度/模版缓冲区及其视图：深度缓冲区其实是一种2D纹理，填写`D3D12_RESOURCE_DESC`结构体来描述纹理资源，所以通过`CreateCommittedResource()`方法去创建它，会创建一个资源与一个**堆**，并将资源提交到这个堆中，资源还要设置初始状态、初始值
- 设置视口：`mCommandList->RSSetViewport()`
- 设置裁剪矩形

对于双缓冲，我们需要2个RTV和1个DSV => 为什么只需要一个DSV？因为每帧开始我们都会清理深度缓冲，所以不会担心上一帧的深度数据会影响到当前帧

堆：GPU资源存储在堆上，本质是一个GPU显存块，有常见的三种类型的堆

- **默认堆**：`D3D12_HEAP_TYPE_DEFAULT` 只有GPU可以访问它 => 深度/模版缓冲区应该放入默认堆中，因为只需要GPU读取
- 上传堆：`D3D12_HEAP_TYPE_UPLOAD` 需要从CPU上传到GPU的资源
- 回读堆：`D3D12_HEAP_TYPE_READBACK` 需要由CPU读取的资源

#### 5.Direct3D绘制过程

顶点数据：

- **输入布局描述**：语义名、语义索引、格式、偏移量 => 类似于OpenGL的VAO
- 顶点缓冲区：
